    ...
    def execute(self, context):
        ...
        for i, table in enumerate(self.tables):
            ...
            if len(records) < 1 or len(records[0]) < 1:
                raise ValueError("{} Data quality check failed. {} returned no results".format(i, table))
            num_records = records[0][0]
            if num_records < 1:
                raise ValueError("{} Data quality check failed. {} contained 0 rows".format(i, table))
