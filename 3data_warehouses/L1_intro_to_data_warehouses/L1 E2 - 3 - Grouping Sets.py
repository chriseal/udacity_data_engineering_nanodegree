...
get_ipython().run_cell_magic('sql', '', 'SELECT SUM(sales_amount)\nFROM factsales')
...
get_ipython().run_cell_magic('sql', '', 'SELECT c.country, SUM(f.sales_amount) AS revenue\nFROM factsales f\n    JOIN dimcustomer c ON (c.customer_key=f.customer_key)\nGROUP BY (c.country)\nORDER BY revenue DESC\nLIMIT 5;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT d.month, SUM(f.sales_amount) AS revenue\nFROM factsales f\n    JOIN dimdate d ON (d.date_key=f.date_key)\nGROUP BY (d.month)\nORDER BY revenue DESC\nLIMIT 5;')
